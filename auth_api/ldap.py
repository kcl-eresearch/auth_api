...
def init_ldap():
    try:
        ...
        ldap.set_option(ldap.OPT_X_TLS_CACERTFILE, current_app.config["ldap"]["ca_file"])
        ...
        ldapc.simple_bind_s(current_app.config["ldap"]["bind_dn"], current_app.config["ldap"]["bind_pw"])
    except Exception:
...
def get_ldap_user(username):
    ldapc = init_ldap()
    filter = "(&(objectClass=user)(sAMAccountName=%s)(!(memberOf:1.2.840.113556.1.4.1941:=%s)))" % (username, ldap.filter.escape_filter_chars(current_app.config["ldap"]["blocked_users_group"]))
    result = ldapc.result(ldapc.search(current_app.config["ldap"]["base_dn"], ldap.SCOPE_SUBTREE, filter, ["mail", "givenName", "sn", "userAccountControl", "sAMAccountName"]))
    ldapc.unbind_ext_s()
